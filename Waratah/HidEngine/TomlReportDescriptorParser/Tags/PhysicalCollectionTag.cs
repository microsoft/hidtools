...
namespace Microsoft.HidTools.HidEngine.TomlReportDescriptorParser.Tags
{
    ...
    /// <summary>
    /// Represents an Physical Collection TOML tag.
    /// </summary>
    [TagAttribute(TagKind.Section, "physicalCollection", typeof(Dictionary<string, object>[]))]
    [SupportedHidKinds(HidUsageKind.CP, HidUsageKind.UM, HidUsageKind.US)]
    public class PhysicalCollectionTag : BaseTag, IModuleGeneratorTag
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="PhysicalCollectionTag"/> class.
        /// </summary>
        /// <param name="usage">UsageTag associated with this ApplicationCollection.</param>
        /// <param name="tags">Tags and (sub) CollectionTags in this collection.</param>
        /// <param name="rawTag">Root TOML element describing a non-ApplicationCollection collection.</param>
        private PhysicalCollectionTag(UsageTag usage, UsageTransformTag usageTransform, List<IModuleGeneratorTag> tags, KeyValuePair<string, object> rawTag)
            : base(rawTag)
        {
            this.Usage = usage;
        ...
        /// <summary>
        /// Gets the associated <see cref="IModuleGeneratorTag"/>s and (sub) <see cref="PhysicalCollectionTag"/>s for this collection.
        ...
        /// <summary>
        /// Attempts to parse the given tag as an <see cref="PhysicalCollectionTag"/>.
        /// </summary>
        /// <param name="rawTag">Root TOML element describing a non-ApplicationCollection collection.</param>
        /// <returns>New <see cref="PhysicalCollectionTag"/> or null if it cannot be parsed as one.</returns>
        ...
        public static PhysicalCollectionTag TryParse(KeyValuePair<string, object> rawTag)
        {
            if (!IsValidNameAndType(typeof(PhysicalCollectionTag), rawTag))
            {
                return null;
            }
            ...
            foreach (KeyValuePair<string, object> child in collectionChildren)
            {
                ...
                if (usage == null)
                {
                    usage = UsageTag.TryParse(child, typeof(PhysicalCollectionTag));
                    if (usage != null)
                    {
                        continue;
                    }
                ...
                {
                    usageTransform = UsageTransformTag.TryParse(child, typeof(PhysicalCollectionTag));
                    if (usageTransform != null)
                    {
                        continue;
                    }
            ...
            return new PhysicalCollectionTag(usage, usageTransform, tags, rawTag);
        }
        ...
        public BaseModule GenerateDescriptorModule(BaseModule parent)
        {
            try
            {
                CollectionModule collection = new CollectionModule(HidConstants.MainItemCollectionKind.Physical, parent);
